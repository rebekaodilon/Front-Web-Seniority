        ...
        <CCardBody>
            ...
            <CRow class="card-option-table">
                <CCol md="12 mb-4" class="text-left">
                    <CButton size="" class="btn-outline-primary uppercase" @click="navigate('/admin/usuario/criar')">
            ...
            <template #show_details="{item}">
                <td class="py-2 text-center">
                    <CButton
                        ...
                        square
                        size="sm"
                        @click="navigate('/admin/usuario/'+ item.IdUsuario)"
                    >
                        Editar
                    </CButton>
                </td>
            ...
            async popularTabela() {
                ...
                if (result.Registers) {
                    result.Registers.forEach((item, index, array) => {
                        array[index].CriadoEm = moment.utc(item.CriadoEm, 'YYYY-MM-DD').local().format('DD/MM/YYYY');
                        array[index].Residencia = item.Residencia ? item.Residencia.Descricao : '';
                    });
