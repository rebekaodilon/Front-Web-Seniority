    ...
    methods: {
        ...
        async criarPlaca() {
            ...
            if (valido == true) {
                ...
                const method = `admin/placa/criar/${this.$route.params.id}`
                const result = await store.actions.postItem(method, data);

                if (!result.Message) {
                    noty.commit("addToaster", {
                        text: "Placa editada com sucesso!",
                        type: "Success"
                    });
        ...
        async getPlaca(){
            try{
                console.log('entrou')
                const method = `admin/placa/${this.$route.params.id}`
                const result = await store.actions.getItem(method);

                if(result){
                    this.Numero = result.Numero;
                    this.ResidenciaSelecionada = result.Residencia;
                }
